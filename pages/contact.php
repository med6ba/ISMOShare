...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nom_complet)) {
        array_push($erreurs, "Le nom complet est obligatoire.");
    }

    if (empty($email)) {
        array_push($erreurs, "L'email est obligatoire.");
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        array_push($erreurs, "L'adresse email n'est pas valide.");
    }

    if (empty($message)) {
        array_push($erreurs, "Le message est obligatoire.");
    }
    ...
    if (empty($erreurs)) {
        ...
        if ($stmt->execute()) {
            ...
            array_push($erreurs, "Une erreur est survenue lors de l'envoi du message. Veuillez réessayer.");
        }
        $stmt->close();
    }
}
