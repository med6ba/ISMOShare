...
// Traiter l'approbation d'un utilisateur
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_user'])) {
    ...
    if ($stmt->execute()) {
        ...
        $notification = "Votre compte a été approuvé. Vous pouvez maintenant vous connecter.";
        $stmt = $conn->prepare("INSERT INTO notification (contenue, date_notification) VALUES (?, CURRENT_DATE)");
        $stmt->bind_param("s", $notification);
        $stmt->execute();
        $notification_id = $conn->insert_id;
        
        // Associer la notification à l'utilisateur
        $stmt = $conn->prepare("INSERT INTO recoit (id_utilisateur, id_notification) VALUES (?, ?)");
        $stmt->bind_param("ii", $user_id, $notification_id);
        $stmt->execute();
    }
}

// Traiter le rejet d'un utilisateur
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reject_user'])) {
    ...
    if ($stmt->execute()) {
        // Créer une notification pour l'utilisateur
        $notification = "Votre demande d'inscription a été rejetée.";
        $stmt = $conn->prepare("INSERT INTO notification (contenue, date_notification) VALUES (?, CURRENT_DATE)");
        $stmt->bind_param("s", $notification);
        $stmt->execute();
        $notification_id = $conn->insert_id;
      ...
      <div class="container-fluid">
        ...
        <?php if ($pending_users->num_rows === 0): ?>
          <div class="text-muted text-center alert alert-info">
            <i class="fas fa-user-clock fa-2x mt-3 mb-2"></i>
