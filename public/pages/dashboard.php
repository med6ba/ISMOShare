...
$stmt = $conn->prepare("SELECT COUNT(*) as count FROM ressource WHERE est_valide = 0");
$stmt->execute();
    ...
    <main class="main-content bg-light py-4">
      <div class="container-fluid">
        ...
        <div class="row g-4">
          ...
          <div class="col">
            <a href="./subpages/valider-reponses.php" 
               class="card shadow-sm h-100 border-0 text-center py-4 px-3 d-flex flex-column align-items-center justify-content-center text-decoration-none bg-white rounded-3 action-card position-relative">
              <i class="fas fa-check-circle fa-2x text-primary mb-3"></i>
              <h5 class="fw-semibold mb-0">Valider les réponses</h5>
              <?php if ($counts['reponses'] > 0): ?>
                <span class="badge-count"><?= $counts['reponses'] ?></span>
              <?php endif; ?>
            </a>
          </div>
        </div>
